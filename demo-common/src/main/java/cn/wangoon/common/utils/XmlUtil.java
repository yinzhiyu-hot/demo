...
import cn.wangoon.common.annotations.XmlElementAnno;
import cn.wangoon.common.constants.BeanXmlConstants;
 ...
 * @PackagePath cn.wangoon.common.utils.XmlUtil
 * @Author YINZHIYU
 * @Date 2020/5/6 15:12
 * @Version 1.0.0.0
 **/
...
public class XmlUtil {
    ...
    public static Object parseXmlToBean(String xml, Class clazz, String matchType) throws Exception {
        if (ObjectUtil.isNotEmpty(xml)) {
            Field[] fields = clazz.getDeclaredFields();//转换对象定义的字段
            List<Field> fieldList = new ArrayList<>();//转换对象定义的字段对应定义的xml节点名称

            for (Field fie : fields) {
                if (StrUtil.equals(BeanXmlConstants.FIELD_ANNOTATION_NAME, matchType)) {
                    if (fie.isAnnotationPresent(XmlElementAnno.class)) {//只解析配置了XmlElementAnno的属性[特殊玩家]
                        fieldList.add(fie);
                    }
                }
                if (StrUtil.equals(BeanXmlConstants.FIELD_NAME, matchType)) {
                        ...
                        if (field.getType().isPrimitive() || field.getType().getName().startsWith("java.lang")) {//基本类型
                            ...
                            if (ObjectUtil.isNotEmpty(childList)) {
                                ...
                                if (fc instanceof ParameterizedType) { // 如果是泛型参数的类型
                                    ParameterizedType pt = CastUtil.cast(fc);
                                    genericClazz = CastUtil.cast(pt.getActualTypeArguments()[0]); //得到泛型里的class类型对象。
                                }
                            ...
                            Element child = null;
                            if (StrUtil.equals(BeanXmlConstants.FIELD_NAME, matchType)) {//根据javabean 属性名称找xml节点
    ...
    public static String apendElement(Element currentElement) {
        ...
        if (ObjectUtil.isNotEmpty(currentElement)) {

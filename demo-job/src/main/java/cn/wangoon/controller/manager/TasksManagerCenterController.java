...
public class TasksManagerCenterController {
    ...
    private List<SyncTaskChartDto> getChartData(List<String> labels) {
        ...
        for (SyncTaskStatusEnum value : SyncTaskStatusEnum.values()) {
            ...
            labels.forEach(date -> {
                SyncTaskChartVO taskChartVO = maps.get(String.format(BaseConstants.STRING_FORMAT_TWO_CONCAT, date, value.getTaskStatus()));
    ...
    private Map<String, SyncTaskChartVO> convertMap(List<SyncTaskChartVO> syncTaskChartVOS) {
        ...
        if (ObjectUtil.isNotEmpty(syncTaskChartVOS)) {
            maps = syncTaskChartVOS.stream().collect(Collectors.toMap(entity -> new StringBuffer()
    ...
    @ResponseBody
    public Result reset(@RequestBody @ApiParam(name = "任务信息", required = true) SyncTask syncTask) {
    ...
    @ResponseBody
    public Result updateTaskData(@RequestBody @ApiParam(name = "任务信息", required = true) SyncTask syncTask) {

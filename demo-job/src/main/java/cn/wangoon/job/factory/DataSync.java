...
/**
 * @Description 数据同步
 * @PackagePath cn.wangoon.service.job.factory.DataSync
 * @Author YINZHIYU
 * @Date 2019-10-28 09:13:00
 * @Version 1.0.0.0
 **/
public abstract class DataSync {
    ...
    public boolean excute(SyncTask syncTask) {
        ...
        try {
            ...
            LogUtils.info(String.format("==> [%s]Thread ID: %s, DataSync 数据同步结束, 同步 %s, 执行耗时 %s ms。", radomStr, Thread.currentThread().getId(), excuteResult ? "成功" : "失败", millis));
        } catch (Exception e) {
